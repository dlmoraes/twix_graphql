defmodule TwixWeb.SchemaTest do
  ...
  describe "users query" do
    test "returns a user", %{conn: conn} do
      user_params = %{nickname: "jose", age: 22, email: "jose@babaninha.com"}
      ...
      expected_response = %{
        "data" => %{"user" => %{"email" => "jose@babaninha.com", "nickname" => "jose"}}
      }
  ...
  describe "users mutation" do
    test "when all params are valid, creates the user", %{conn: conn} do
      query = """
      mutation {
        createUser(input: {nickname: "tufa", email: "tufa@frutas.com", age: 35}) {
          id
          nickname
          age
        }
      ...
      assert %{"data" => %{"createUser" => %{"age" => 35, "id" => _id, "nickname" => "tufa"}}} =
               response
    end
  end
